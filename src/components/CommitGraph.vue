...
const filteredCommits = computed(() => {
  return commitHistory.value
    .filter(commit => {
      if (
        filterAuthor.value &&
        !commit.author.toLowerCase().includes(filterAuthor.value.toLowerCase())
      ) {
        return false
      }
      if (
        filterKeyword.value &&
        !commit.subject.toLowerCase().includes(filterKeyword.value.toLowerCase())
      ) {
        return false
      }
      return true
...
<template>
  <div class="commit-graph">
    ...
    <div v-else-if="filteredCommits.length === 0" class="empty">No commits found</div>

    <div v-else class="commit-list">
      <div v-for="commit in filteredCommits" :key="commit.id"
        :class="['commit-item', { selected: selectedCommits.has(commit.id) }]" @click.ctrl="toggleSelect(commit.id)"
        @click.meta="toggleSelect(commit.id)">
        <!-- 左侧选择框 -->
        <div class="commit-checkbox">
          <input type="checkbox" :checked="selectedCommits.has(commit.id)" @change="toggleSelect(commit.id)" />
        </div>
        ...
        <div class="commit-stats">
          <div v-if="commit.files > 0" class="stat-badge">{{ commit.files }} files</div>
