...
export function useBuiltinCommands() {
   ...
   * value: AI 提示词模板或函数
   *
   * 注意：所有内置命令必须以 : 开头，避免与系统命令或 CLI 工具冲突
   */
  ...
  const isBuiltinCommand = input => {
    ...
    // 只支持带 : 前缀的命令，避免与系统命令冲突
    return Object.prototype.hasOwnProperty.call(builtinCommands, command)
  }
  ...
  const getCommandPrompt = input => {
    ...
    if (!Object.prototype.hasOwnProperty.call(builtinCommands, command)) {
      return null
    }
